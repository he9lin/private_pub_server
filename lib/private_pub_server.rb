...
module PrivatePubServer
  class << self
    ...
    def configure(private_pub_config_file:,
                  run_env:,
                  redis_uri:,
                  redis_ns:,
      ...
      FayeTracking.configure do |config|
        ...
        config.on_subscribe do |client_id, user_id, channel|
          ...
          users = FayeTracking.users_in_channel(channel)
          PrivatePubServer::PresencePublisher.async_publish_presence \
            channel: channel,
            user_id: user_id,
            client_id: client_id,
            users: users
...
require_relative 'private_pub_server/hash_ext'
